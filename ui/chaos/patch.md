...
## New standalone file needed

`csv_to_txt.py` is referenced by patch #5 but was not part of the
original upload set — see the companion `csv_to_txt.py` file, which
mirrors `mps_to_txt.py`'s `convert_file()`/`write_meta()` interface for
CSV input, built on `convert_to_gpu_format.py`'s `canonicalize_csv()`.
Drop it in the same directory as `mps_to_txt.py`, `gpu_format.py`, and
`convert_to_gpu_format.py`.
